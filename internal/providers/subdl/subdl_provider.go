...
func (p *SubDL) SearchSubtitle(ctx context.Context, searchOpts providers.SearchOptions) ([]providers.Subtitle, error) {
	...
	if len(subs) == 0 {
		if searchOpts.IsSerie {
			return nil, fmt.Errorf("no results returned for %v Season %v Episode %v", searchOpts.Query, searchOpts.Season, searchOpts.Episode)
		}
		return nil, fmt.Errorf("no Results returned for %v", searchOpts.Query)
	}
	return subs, nil
}
